...
export const useGameStore = defineStore('game', {
  ...
  actions: {
    ...
    async sendCoins(recipientUsername, amount) {
      ...
      if (!floored || floored <= 0) throw new Error(t('storeErrors.invalidAmount'))
      await this.persist()
      ...
      if (error) throw error
      this.coins = Number(data?.sender_balance ?? this.coins - floored)
      return data
    },
    ...
    async claimCraftAnimal() {
      if (!this.craftJob?.active) throw new Error(t('storeErrors.noCraftJob'))
